...
class MatrixFactorization(object):
    ...
    def __init__(self, R, factors=5, steps=1000, lr=1e-4):
        ...
        self.mask = self.R.astype(int) # TODO (TASK 1)
        self.split = np.random.rand(self.R.shape[0], self.R.shape[1]) # TODO (TASK 1)
        self.mask_train = self.mask * (self.split > 0.2) # TODO (TASK 1)
        self.mask_test = self.mask * (self.split < 0.2) # TODO (TASK 1)
    ...
    def gradient_user_matrix(self, error):
        return -2 * np.matmul((self.mask * error), self.V) # TODO (TASK 2)

    def gradient_movie_matrix(self, error):
        return -2 * np.matmul((self.mask * error).T, self.U) # TODO (TASK 2)

    def update_user_matrix(self, u_grad):
        self.U = self.U - self.lr * u_grad # TODO (TASK 3)

    def update_movie_matrix(self, v_grad):
        self.V = self.V - self.lr * v_grad # TODO (TASK 3)
    ...
    def fit(self):
        for i in range(self.steps):
            ...
            error = self.R - np.matmul(self.U, self.V.T) # TODO (TASK 2) # Compute the error outside the gradient computation, so we don't have to do it twice
