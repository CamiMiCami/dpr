...
class FaceDetector(object):
    ...
    def detect(self, image):
        ...
        def sliding_window(hog, hog_size, cell_size, step_size):
            for y in range(0, hog.shape[0] - hog_size, step_size // cell_size):
                for x in range(0, hog.shape[1] - hog_size, step_size // cell_size):
                    yield (x * cell_size, y * cell_size, hog[y:y+hog_size, x:x+hog_size])

        boxes = []
        confidences = []

        cell_size = self.hog_config["pixels_per_cell"][0]
        hog_size = self.template_size // cell_size - self.hog_config["cells_per_block"][0] + 1

        for resized in pyramid(image, scale=self.pyramid_scale, template_size=self.template_size):
            scale = resized.shape[0] / image.shape[0]
            hog = extract_hog_features(resized, self.hog_config)
            for (x, y, window) in sliding_window(hog, hog_size, cell_size, self.step_size):
                confidence = self.classifier.decision_function(window.reshape(1, -1))[0]
                if confidence >= self.confidence_threshold:
                    x_min = int(x / scale)
                    y_min = int(y / scale)
                    x_max = int((x + self.template_size) / scale)
                    y_max = int((y + self.template_size) / scale)
