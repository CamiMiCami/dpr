...
@ray.remote
def distributed_pos_feature_extraction(file, template_size, hog_config):
    image = read_file(file)
    image = resize(image, output_shape=(template_size, template_size))
    features = extract_hog_features(image, hog_config)
    return features


@ray.remote
def distributed_neg_feature_extraction(file, template_size, hog_config, scale, samples_per_scale):
    image = read_file(file)
    features = []

    def pyramid(image, scale, template_size):
        yield image
        # keep downscaling image until it is smaller than template_size
        while True:
            image = rescale(image, scale, multichannel=True)
            if np.min(image.shape[:2]) <= template_size:
                break
            yield image

    def extract_patch(image, template_size):
        randx = np.random.randint(0, image.shape[1] - template_size)
        randy = np.random.randint(0, image.shape[0] - template_size)
        if len(image.shape) == 3:
            image = image[randy:randy+template_size, randx:randx+template_size, :]
        else:
            image = image[randy:randy + template_size, randx:randx + template_size]
        return image


    for resized in pyramid(image, scale=scale, template_size=template_size):
        for _ in range(samples_per_scale):
            patch = extract_patch(resized, template_size=template_size)
            features.append(extract_hog_features(patch, hog_config))
...
def get_positive_features(files, template_size, hog_config):
    ...
    futures = [distributed_pos_feature_extraction.remote(file, template_size, hog_config) for file in files]
    features = np.asarray(ray.get(futures))
...
def get_negative_features(files, template_size, hog_config, scale, samples_per_scale):
    ...
    # Extract hog features for all files
    start_time = time.time()
    futures = [distributed_neg_feature_extraction.remote(file, template_size, hog_config, scale, samples_per_scale) for file in files]
    features = ray.get(futures)
    features = np.asarray([feature for subfeatures in features for feature in subfeatures])
    features = features.reshape(features.shape[0], -1)
    logging.debug(f"Feature extraction took {int(time.time() - start_time)} seconds.")
